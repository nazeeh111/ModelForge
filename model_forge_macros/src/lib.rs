...
pub fn parse_simple_polynomial(input: TokenStream) -> TokenStream {
    ...
    let mut tokens = String::from("::model_forge::polynomials::SimplePolynomial { ");
...
pub fn parse_intermediate_polynomial(input: TokenStream) -> TokenStream {
    ...
    let mut tokens =
        String::from("::model_forge::polynomials::IntermediatePolynomial { ");
    tokens.push_str("terms: vec![");
    for term in output.terms {
        tokens.push_str(&format!(
            "::model_forge::polynomials::Term {{ coefficient: {:?}, variables: vec![",
            term.coefficient,
        ));
